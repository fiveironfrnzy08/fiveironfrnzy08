...
public class Eiscp {
	...
	private static String						CLASSNAME				= "com.tom.onkyo.Eiscp";
	...
	private static final String					DEFAULT_EISCP_IP		= "192.168.1.108";
	...
	/**
	 * Simple class Constructor (using deafult IP and port) that gets all the
	 * class command constants set-up along with their command lookup maps
	 * (commandNameMap_ and commandMap_) .
	 **/
	public Eiscp( ) {
		initCommandMap( );
	}
	...
	/**
	 * Constructor that takes your receivers ip and default port, gets all the
	 * class command constants set-up along with their command lookup maps
	 * (commandNameMap_ and commandMap_) .
	 **/
	...
	/**
	 * Constructor that takes your receivers ip and port, gets all the class
	 * command constants set-up along with their command lookup maps
	 * (commandNameMap_ and commandMap_) .
	 **/
	...
	/**
	 * Tests the Connection to the receiver by opening a socket connection
	 * through the DEFaULT IP and DEFAULT eISCP port.
	 **/
	public boolean testConnection( ) {
		return testConnection( DEFAULT_EISCP_IP, DEFAULT_EISCP_PORT );
	}
	 ...
	 * 
	 * @param String
	 *            holding the HEX string to convert back to decimal
	 * @return a string holding the HEX representation of the passed in str.
	 **/
	 ...
	 * 
	 * @param String
	 *            holding the HEX string to convert backk to decimal
	 * @param boolean flag to turn some debug output on/off
	 * @return a string holding the HEX representation of the passed in str.
	 **/
	...
	public StringBuilder getEiscpMessage( int command ) {
		...
		StringBuilder sb = new StringBuilder( );
		int eiscpDataSize = ( ( String ) commandMap_.get( Integer.valueOf( command ) ) ).length( ) + 2; // this
																										// is
																										// the
																										// eISCP
																										// data
																										// size
		int eiscpMsgSize = eiscpDataSize + 1 + 16; // this is the eISCP data
													// size
	...
	public void sendCommand( int command, boolean closeSocket ) {
		...
		if ( connectSocket( ) ) {
			try {
				...
				out_.writeBytes( sb.toString( ) ); // <--- This is the one that
													// works
	 ...
	 * and returns all response packetMessages <br />
	...
	/**
	 * Sends to command to the receiver and then waits for the response(s) <br />
	 * and returns all response packetMessages.
	 * 
	 * @param command
	 *            must be one of the Command Class Constants from the
	 *            eiscp.Eiscp.Command class.
	...
	public Vector< String > readQueryResponses( ) {
		...
		if ( connected_ ) {
			try {
				...
				eiscpSocket_.setSoTimeout( socketTimeOut_ ); // this must be set
																// or the
																// following
																// read will
																// BLOCK / hang
				...
				while ( ( numBytesReceived = in_.read( responseBytes ) ) > 0 ) {
					...
					// *******************************************
					char[] responseChars = msgBuffer.toString( ).toCharArray( ); // use
																					// the
																					// charArray
																					// to
																					// step
					...
					while ( responseByteCnt < totBytesReceived ) {
						...
						dataStartChar = responseChars[ responseByteCnt++ ]; // parse
																			// and
																			// throw
																			// away
																			// (like
						...
						if ( endChar1 == ( char ) Integer.parseInt( "1A", 16 ) && endChar2 == ( char ) Integer.parseInt( "0D", 16 ) && endChar3 == ( char ) Integer.parseInt( "0A", 16 ) )
							if ( debugging )
	...
	public static String getVolumeCmdStr( ) {
		return "MVL" + Integer.toHexString( volume_ );
	}
	 ...
	 **/
	private static String getHelpMsgStr( ) {
		return getHelpMsg( ).toString( );
	}

	/**
	 * initializes and gets the helpMsg_ class var.
	 * 
	...
	public static void main( String[] args ) {
		...
		Eiscp instance = new Eiscp( ); // Optional constructors to specify IP/Port
...
//				instance.sendCommand( UNMUTE );
				...
				int command = -1;
				String commandStr = "";
				// TODO: Set up a loop to handle multiple commands/args in one
				// run with one socket connection
				command = instance.getCommand( args[ 0 ].toUpperCase( ) ); // returns
																			// -1
																			// if
																			// not
																			// found
				...
				if ( command == VOLUME_SET )
					instance.setVolume( Integer.parseInt( args[ 1 ] ) );
	 ...
	 * 
	 * @param volume
	 *            the value to set the class volume_
	 **/
	...
	public String getCommandName( String commandStr ) {
		...
		for ( int i = 0; i < eCnt; i++ ) {
			if ( ( ( String ) commandMap_.get( i ) ).equals( commandStr ) )
				command = i;
		}
		if ( command != -1 ) {
	...
	/**
	 * searches for the command that is associated with the passed command
	 * constant.
	 * 
	 * @param command
	 *            the command referencing the iscp command constant
	 ...
	 * 
	 * @param commandName
	 *            the commandName key referencing the iscp command str
	 * @return the iscp command string (example 'SLI10')
	 **/
	...
	private void initCommandMap( ) {
		...
		// commandNameMap_.put("ZONE2_SOURCE_OFF", ZONE2_SOURCE_OFF); // not
		// supported
	 ...
	 * </UL>
	 **/
	public static final int	NETUSB_PLAY_STATUS_QUERY		= eCnt++;	// NET/USB
																		// Track
																		// Info
																		// (Current
																		// Track/Toral
