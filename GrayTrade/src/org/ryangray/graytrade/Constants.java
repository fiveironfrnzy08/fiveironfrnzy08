package org.ryangray.graytrade;

public class Constants {

	public static final String	SERVER			= "jdbc:mysql://ryangray.org:3306";
	public static final String	SERVER_USER		= "root";
	public static final String	SERVER_PASS		= "ryan135244";
	public static final String	DATABASE		= "master";
	
	public static final String	PROGRAM_NAME	= "Gray Trade";
	
	public static final String	LTC_BTC_DATA_TABLE		= "graytrade";
	public static final String	LTC_BTC_INFO_TABLE		= "graytrade_info";
	public static final String	LTC_BTC_TRADES_TABLE		= "graytrade_trades";
	
	public static final String	DOGE_BTC_PING		= "btc_doge_ping";
	public static final String	DOGE_BTC_INFO		= "btc_doge_info";
	public static final String	DOGE_BTC_ORDER		= "btc_doge_order";
	

}
